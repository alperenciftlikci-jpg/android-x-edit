...
licensee {
    ...
    // NextLib (GPL-3.0): FFmpeg-based software video decoders that handle profiles
    // the platform MediaCodec rejects with NO_EXCEEDS_CAPABILITIES — H.264 Hi10P,
    // HEVC Main 10, HEVC 4:2:2 10-bit. GPL-3.0 is compatible with this project's
    // AGPL-3.0 distribution path; the explicit allowDependency keeps the broader
    // GPL-3.0 gate closed so unrelated GPL packages can't slip in unnoticed.
    allowDependency("io.github.anilbeesetti", "nextlib-media3ext", "1.10.0-0.12.1") {
        because("GPL-3.0 wrapper for FFmpeg decoders; AGPL-3.0 compatible.")
    }
